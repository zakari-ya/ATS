import Link from "next/link";
import { AlertCircle, History, Plus } from "lucide-react";

import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { RetryScanButton } from "@/features/scan/components/retry-scan-button";

type ResultFailedStateProps = {
  scanId: string;
  title: string;
  message: string;
  canRetry: boolean;
};

export function ResultFailedState({
  scanId,
  title,
  message,
  canRetry,
}: ResultFailedStateProps) {
  return (
    <Card className="border-[#edcbc7] bg-white shadow-sm shadow-[#183f3a]/5">
      <CardContent className="flex flex-col gap-6 p-5 sm:p-7">
        <div className="space-y-4">
          <div className="flex size-12 items-center justify-center rounded-2xl bg-[#fff7f7] text-[#8a2d2d]">
            <AlertCircle className="size-5" aria-hidden="true" />
          </div>
          <div className="space-y-3">
            <Badge className="w-fit bg-[#fff1f1] text-[#8a2d2d] hover:bg-[#fff1f1]">
              Failed scan
            </Badge>
            <h2 className="text-2xl font-semibold tracking-tight text-[#183f3a]">
              {title}
            </h2>
            <p className="text-base leading-7 text-[#66736f]">
              {message}
            </p>
          </div>
        </div>

        {!canRetry ? (
          <Alert className="border-[#f0d7c5] bg-[#fffaf4] text-[#8b5e27]">
            <AlertCircle className="size-4" aria-hidden="true" />
            <AlertTitle>Retry unavailable</AlertTitle>
            <AlertDescription className="text-[#8b5e27]">
              The original CV file is not available, so this scan cannot be
              retried from here. Start a new scan with a clean text-based CV
              PDF.
            </AlertDescription>
          </Alert>
        ) : null}

        <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
          {canRetry ? <RetryScanButton scanId={scanId} /> : null}
          <Button
            asChild
            className="h-11 rounded-xl bg-[#183f3a] px-4 text-white hover:bg-[#1f4d47]"
          >
            <Link href="/scan">
              <Plus className="size-4" aria-hidden="true" />
              New scan
            </Link>
          </Button>
          <Button
            asChild
            variant="outline"
            className="h-11 rounded-xl border-[rgba(31,77,71,0.14)] bg-white px-4 text-[#183f3a] hover:bg-[#eef4f2]"
          >
            <Link href="/history">
              <History className="size-4" aria-hidden="true" />
              Back to history
            </Link>
          </Button>
        </div>
      </CardContent>
    </Card>
  );
}
